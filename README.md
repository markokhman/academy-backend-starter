...
- ✅ Прогнать end-to-end (миниап → /api/auth/verify → профиль вернулся).
- Убрать DEV-bypass для mock-hash в продакшене (он отключается по `ENVIRONMENT`).
- Кастомный OpenAPI со scrubbing для прода (есть в `appss-back`, для академии вырезано).
...
- Деплой: **Railway / Fly.io / Render** (везде через `Dockerfile`).

---

## Часть программы

Этот репозиторий — артефакт **Кола 2** программы Vibe Coding Academy («Подготовка + первый запуск»). На этом коле ученик поднимает связку. На колах 3-6 поверх пишутся API эндпоинты и бизнес-логика.
